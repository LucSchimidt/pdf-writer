...
interface Imagem {
  ...
  origin:string;
  origin_url?: string;
  hash: String[]; //Array de strings para cada hash
}
...
interface AboutPage {
  ...
  registeredContentTypes: string;
  websiteTechnicalCapture: string;
  fileIntegrityWithHashCodes: string;
  pdfAReport: string;
  digitalSignatureAndTimestampICP: string;
  complementaryServices: string;
  registrationScopeAndObject: string;
  technicalSecurityAndCompliance: string;
  registrationValidation: string;
  realdocValidator: string;
  manualValidation: string;
  essentialLegalAspects: string;
}
...
class GenerateReport {
  private doc: PDFKit.PDFDocument;
    ...
    introductionPage(introduction:string) {
    ...
    certificationPage(qrcode_path: string, code:string, validation_url:string) {
      this.doc.addPage();
      ...
      this.doc.fillColor('#4B5563');
      this.doc.font('Helvetica')
      this.doc.fontSize(10).text('* As assinaturas certificadas constam no documento digital original somente. Modificações ou a impressão do documento podem inviabilizar sua validação, veja mais no item 3 deste documento.'
        , 75, start_y)
    ...
    registerDetails(details:RegisterDetails) {
      this.doc.addPage();
    ...
    addImage(image:Imagem) {
      ...
      const resultado: string = image.hash
        .map((hash) => `HASH = ${hash}`)
        .join(', ');
    ...
    aboutPage(about_content:AboutPage) {
      this.doc.addPage({layout:'portrait'})
...
function geraRelatorio() {
  ...
  const detalhes_do_registro:RegisterDetails = {id: '90123j1209ujed182ji123', start_date: '2024-04-03', end_date:'2024-04-07', session_time:200, utf:'(UTC-03:00) Brasilia', ambiente:'WEBSITE - Ponto(s) de acesso à internet: 45.170.27.220', packages:[pacote_um]}
  meu_relatorio.registerDetails(detalhes_do_registro)


  //Adicionando imagens:
  const imagem_um:Imagem = {archive_name: 'minha_imagem.png', size: 20.0,  path: './assets/captura.png', date: '2024-12-24', origin: 'google.com', origin_url: 'https://www.google.com', hash:['hash_exemplo_um11111', 'hash_exemplo_dois2222']}
